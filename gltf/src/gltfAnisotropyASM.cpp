    ...
    for (size_t y = 0; y < anisotropyImage.height; ++y) {
        ...
        for (size_t x = 0; x < anisotropyImage.width; ++x) {
            ...
            anisoLevelPixels[y * anisotropyImage.width + x] =
              calculateASMLevel(blueChannelValue * strength, roughness);
    ...
    for (size_t y = 0; y < roughnessImage->height; ++y) {
        ...
        for (size_t x = 0; x < roughnessImage->width; ++x) {
            ...
            anisoLevelPixels[y * roughnessImage->width + x] =
              calculateASMLevel(strength, roughness);
        }
    }
}
                        ...
                        std::unordered_map<std::string, int>& cache)
{
    ...
    if (anisotropyData.texture.index >= 0) {
        levelCacheKey = generateAnisotropyImageName(
          AdobeTokens->anisotropyLevelTexture.GetText(), strength, anisotropyData.rotation);
        angleCacheKey = generateAnisotropyImageName(
          AdobeTokens->anisotropyAngleTexture.GetText(), strength, anisotropyData.rotation);
    ...
    if (ctx.usd != nullptr) {
        if (m.anisotropyLevel.value.IsEmpty() && m.anisotropyLevel.image < 0) {
            if (m.anisotropyAngle.value.IsEmpty() && m.anisotropyAngle.image < 0) {
                // Return if there is no anisotropy data so as to not write out an empty
                // extension
                return;
            }
        }
        tinygltf::ExtensionMap ext;
        float anisotropyLevelValue = 1.0f;
        float anisotropyAngleValue = 0.0f;
        float reconstructedStrength = 1.0f;
        float reconstructedAngle = 0.0f;
        bool hasAnisotropyLevelImage = false;
        if (m.anisotropyLevel.value.IsHolding<float>()) {
            ...
            addStrengthToAnisotropyExt(ext, reconstructedStrength);
        } else if (m.anisotropyLevel.image >= 0) {
            hasAnisotropyLevelImage = true;
        }

        bool hasAnisotropyAngleImage = false;
        if (m.anisotropyAngle.value.IsHolding<float>()) {
            reconstructedAngle =
              reverseASMRotation(m.anisotropyAngle.value.UncheckedGet<float>(), 0.0f);
            addRotationToAnisotropyExt(ext, reconstructedAngle);
        } else if (m.anisotropyAngle.image >= 0) {
            hasAnisotropyAngleImage = true;
        }

        // if both level and angle are stored in images, we need to construct a combined anisotropy
        // image to export, as glTF only supports one texture for anisotropy
        if (hasAnisotropyLevelImage || hasAnisotropyAngleImage) {
            ...
            std::string constructedTextureUri = constructedTextureName + ".png";

            ExportTextureCacheItem& exportTextureCacheItem =
              constructedAnisotropyTextureCache[constructedTextureUri];

            int textureIndex = -1;
            int texCoord = -1;
            if (exportTextureCacheItem.textureIndex < 0) {
                Image constructedImage;
                constructedImage.channels = 3;

                // If there is no anisotropy level or angle image, we will use a default 1x1 image
                // with the value set to the non-image anisotropy data, so that we can still export
                // the anisotropy extension and have it be read correctly by other applications,
                // rather than just exporting an empty extension with no texture which would be
                // ambiguous for a glTF importer to interpret
                Image defaultAnisotropyLevelImage;
                Image defaultAnisotropyAngleImage;
                if (!hasAnisotropyLevelImage) {
                    defaultAnisotropyLevelImage.allocate(1, 1, 1);
                    defaultAnisotropyLevelImage.pixels[0] = anisotropyLevelValue;
                }
                if (!hasAnisotropyAngleImage) {
                    defaultAnisotropyAngleImage.allocate(1, 1, 1);
                    defaultAnisotropyAngleImage.pixels[0] = anisotropyAngleValue;
                }
                ...
                Image& anisotropyLevelImage = hasAnisotropyLevelImage && decodedLevel.first
                                                ? decodedLevel.second
                                                : defaultAnisotropyLevelImage;
                Image& anisotropyAngleImage = hasAnisotropyAngleImage && decodedAngle.first
                                                ? decodedAngle.second
                                                : defaultAnisotropyAngleImage;
                ...
                constructedAnisotropyInput.image =
                  inputTranslator.addImage(std::move(constructedImage),
                                           constructedTextureName,
                                           constructedTextureUri,
                                           ImageFormatPng,
                                           false);
                exportTexture(ctx, constructedAnisotropyInput, textureIndex, texCoord);
                if (textureIndex != -1 && texCoord != -1) {
                    exportTextureCacheItem.textureIndex = textureIndex;
                    exportTextureCacheItem.texCoord = texCoord;
                }

            } else {
                textureIndex = exportTextureCacheItem.textureIndex;
                texCoord = exportTextureCacheItem.texCoord;
            }
            if (textureIndex != -1 && texCoord != -1) {
