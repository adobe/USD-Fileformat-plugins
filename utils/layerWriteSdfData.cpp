               ...
               const std::string& sourceFileType)
{
    ...
    comment += "USD version: " TOSTRING(PXR_VERSION) "\n";
    comment += "Date: " + getCurrentDate() + "\n";
    comment += "Source File Type: " + sourceFileType + "\n";
#ifdef PRODUCT_STRING
    comment += "Product: " TOSTRING(PRODUCT_STRING) "\n";
#endif

    setLayerMetadata(sdfData, SdfFieldKeys->Comment, VtValue(comment));
...
_writePrimvars(SdfAbstractData* sdfData, const SdfPath& primPath, const Mesh& mesh)
{
    ...
    if (mesh.asGsplats) {
        ...
        if (mesh.clippingBox.values.size() >= 2) 
        {
            ...
            // Also modify the extent attribute so that the USD bounding box can be correctly computed.
                   ...
                   const std::string& debugTag)
{
    ...
    _writeMetadata(sdfData, usdData, rootNodePath, sourceFileType);
